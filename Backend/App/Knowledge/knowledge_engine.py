...
class KnowledgeEngine:
    ...
    def _load_database(self):
        """Loads all vectors from MSSQL into a Numpy Matrix for fast math."""
        try:
            with engine.connect() as conn:
                # Fetch everything
                query = text("SELECT content, embedding, category, source_section FROM knowledge_base")
                ...
                for row in rows:
                    # SQLAlchemy rows can be accessed by index or name.
                    # content=0, embedding=1, category=2, source=3
                    content = row.content
                    blob = row.embedding
